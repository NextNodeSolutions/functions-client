...
describe('OptimizedBackgroundImage URL Validation', () => {
	const createMockImage = (src: string): OptimizedImageType => ({
		src,
		width: 800,
		height: 600,
		format: 'jpg',
	...
	describe('javascript: protocol injection', () => {
		it('should reject javascript: protocol in image.src', () => {
			...
			expect(() =>
				render(
					<OptimizedBackgroundImage image={maliciousImage}>
						Content
					</OptimizedBackgroundImage>,
				),
			).toThrow('Invalid image URL: javascript: protocol not allowed')
		})
	...
	describe('data URI validation', () => {
		it('should accept valid data:image/* URIs', () => {
			...
			expect(() =>
				render(
					<OptimizedBackgroundImage image={validImage}>
						Content
					</OptimizedBackgroundImage>,
				),
			).not.toThrow()
		...
		it('should reject data:text/html URIs', () => {
			...
			).toThrow('Invalid image URL: only data:image/* allowed')
		})
	...
	describe('relative paths', () => {
		...
		it('should reject relative paths without leading /', () => {
			...
			expect(() =>
				render(
					<OptimizedBackgroundImage image={invalidImage}>
						Content
					</OptimizedBackgroundImage>,
				),
			).toThrow(
